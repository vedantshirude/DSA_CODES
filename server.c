...
{
    stack_t* p_stack=create_list();
    return p_stack;
}
status_t push(stack_t*p_stack,data_t p_data)
{
    status_t status= insert_end(p_stack,p_data);
    return(status);
}
status_t pop(stack_t*p_stack,data_t*p_data)
{
    status_t status=pop_end(p_stack,p_data);
    return(status);
}
status_t peek(stack_t*p_stack,data_t*p_data)
{
    status_t status=get_last(p_stack,p_data);
    return(status);
}
bool is_stack_empty(stack_t*p_stack)
{
    bool status=is_list_empty(p_stack);
    return(status);
}
len_t total_stack_element(stack_t*p_stack)
{
    len_t length=get_length(p_stack);
    return(length);
}
